...
2番目と3番目のコマンドの結果について、initrc_domain と daemon と direct_init_entry はタイプでなく属性 (Attribute) です。
...
- **staff_r** : ユーザの切り替えができる制限されたロール。このロールに属するSEユーザは、newrole コマンドによるロールの切り替えができます。/etc/sudoers で実行できるコマンドを指定するときにセキュリティコンテキストも指定しておけば、管理者権限も使用できるようになります。
