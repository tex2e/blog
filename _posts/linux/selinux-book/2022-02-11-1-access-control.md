...
LinuxのDACによるアクセス制御には、問題点があります。
それは、高い権限を必要とするプログラムやサービスが実行可能および稼働しているとき、そのプログラムやサービスを利用して権限昇格をされる可能性がある、という問題点です。
...
しかし、user2 から root ユーザに権限昇格した場合は、/etc/passwd に書き込むことができます。
...
RBAC (Role-Based Access Control) は、職務や認可に応じて個別にロールを設定し、情報システムへのアクセスを許可されたユーザに限定するアクセス制御システムです。
簡単にいうと、ユーザをロールに所属させて、ロールごとにアクセス制御を行う仕組みのことです。
RBACは、ロールに基づくアクセス制御とも呼ばれます。
RBACの仕組み自体は、一般的な Linux でも使用されています。
