...
主に図、表、数式、アルゴリズム、文章を書くときに気をつけるべきことについてです。
...
- **配列を扱うときに「a[5] := 3」はプログラム言語に依存した書き方なので、代わりに添字を使って「a₅ := 3」と書くこと**
...
- **「が」を逆説ではなく単純接続として使っていないか**（「が」は文を長くする原因となる。たとえば「上の図では一つの最大公約数について示した**が**，これが圏である．」は逆説ではなく単純接続として使われている。文を2つに切るべき）
