...
- アライメントとは、目標や目的を達成するために計画やリソースを調整すること
...
#### 知的財産保護
- **知的財産** (IP; Intellectual Property) とは、人間の創造性による無形の創造物。著作権や商標などのこと
...
- **分散管理** : ファイルのオーナーや作成者が、情報へのアクセスを制御すること
  - 分散型アクセス制御は、ソースに近い人員にアクセス制御の権限を与えること
...
- LDAPのSASL (Simple Authentication and Security Layer) は、セキュアな方式を含む幅広い認証タイプがサポートされる
...
#### 認証プロトコル
- PAP : 平文パスワードによる認証プロトコル
- CHAP : チャレンジレスポンスによる認証プロトコル
- EAP : PKIを使用する拡張認証プロトコル。通常は暗号化されない
  - 事前共有鍵 (EAP-PSK) または TLS (EAP-TLS) を使用して暗号化を行う。Wi-Fiで使用される
- PEAP : EAP-TLSに類似している保護された拡張認証プロトコル
  - EAPメソッド暗号化機能と認証機能を提供する
- LEAP : Cisco版のEAP。WEPで使用される。保護機能に脆弱性があり危険
...
- 境界装置を通過するためには、複数のセグメントに分け、認証を必須にする必要がある
  - 要塞サーバ (Bastion Server) : インフラ領域のサーバへのSSH接続を提供する踏み台サーバ
...
#### ハードウェアの操作
- **ルートオブトラスト** (RoT) : トラストアンカーに依存することで、後続のアクションで作られたシステムはセキュアな状態であることを保障する。ハードウェアやOSのセットアップなど
...
- NAC (Network Access Control) は、ネットワークに参加を希望するデバイスが、ポリシーの要件を満たしているときのみ、接続できる仕組み
...
- エンドポイントセキュリティのソリューションの主要な機能：
  ...
  - デバイスのジオロケーション（位置情報）
- エンドポイントで取得される膨大なデータ (ログ) を扱えるようにすること
...
#### ポートアドレス変換 (PAT)
- あらゆるアドレスを1個のIPアドレスに変換するNATの拡張版
...
#### プロキシの種類
- サーキットレベルのプロキシは、通信のオーバーヘッドは少ないが、悪意のあるコンテンツは検出できない
...
- VoIP
  ...
  - SIP : マルチメディア接続を管理するためのプロトコル。MD5によるダイジェスト認証と完全性保護を提供する
- Wi-Fi
  - IEEE 802.11 スタンダードの準拠した無線ネットワーク通信
  - LEAP方式では、認証が成立すると、クライアント固有のWEPキーが発行される
  - WPA2 PSKでは、パスワードを共有してもデータ収集はできない
...
- ワイヤレス攻撃への対策は、物理的・管理的・技術的なものがある
  ...
  - ゲスト用Wi-Fiデバイスを使用できる物理的エリアに関する管理ポリシー
  - WAPを規則に基づいて管理し、通常の業務時間外の不正使用を防止する
...
- L2TPはIPsecを使用してトラフィックを暗号化する（L2TP VPN）
- PPTPは、PPPを基盤として、PAP, CHAP, EAP または IPsecで認証する
- L2でトンネリングするため、非IPプロトコルの通信にも対応している
...
- 以下基本的なメカニズム：
  + P2Pアプリケーション
    ...
    - ボットネットを管理するマスターノードをシャットダウンしても、P2Pボットネットを制御する「ボットハーダ(bot herder)」はスレーブノードの1つをマスタノードにすることで、ボットネット操作を継続する
  + インスタントメッセージ
...
- 概念
  ...
  - **データ保護**や**データ隠蔽** : データの読み取りや変更を制限・防止すること
  - **コードプロテクション**や**ロジック隠蔽** : あるソフトウェアが別のソフトウェアのソースコード・中間コード・実行コードを読み取ったり変更したりするのを防止すること
...
- これにより、アプリケーションはリモートマシンからローカルホストにコードをダウンロードできる
...
#### ソースコードを書くのは誰か
- ITサプライチェーンの視点から見て、ソフトウェアの完全性をどのように保証するのか
...
- DDL (Data Definition Languaage) : データベースやテーブル、ビューなどを作成する
- データ操作言語 (DML; Data Manipulation Language) : データの照会や挿入、削除、更新などをする
- データ制御言語 (DCL; Data Control Language) : 管理者がデータへのアクセスを確立・制御する

#### マークアップ言語とデータベース
- データベース接続技術の中でマークアップ言語が使用される

#### アプリとデータベース接続
- プログラムからデータベースに接続する
- インターフェイス技術 (APIなど) を適切に利用することで、より強力なセキュリティと制御、データ接続サービスの強化が可能になる
...
#### 階層的アプリケーション手法
- 最もよく使われるアーキテクチャは、以下の3層構造を採用している：
  - プレゼンテーション層 (View)
...
#### ソーシャルエンジニアリング
- ソーシャルエンジニアリング : だますことと脅すことによって人がすべきでないことをさせること
- ダンプスターダイビング (dumpster driving)  : 捨てられた潜在的から価値のある情報を見つけること
- ショルダーサーフィング (shoulder surfing)  : 人がパスワードを機器に入力する様子を盗み見て情報を不正に入手すること
...
- ソフトウェアのブロックリストと許可リストは、事前に承認されていないファイルを実行しようとする試みを検知・防止し、ITセキュリティ担当者に警告を発する
- 人種差別を考慮して以下の言い換えをすること：
  - ホワイトリスト => 許可リスト
  - ブラックリスト => ブロックリスト

#### マネージドセキュリティサービス
- サードパーティに委託するほうがビジネス上、理にかなっている場合もある

#### ランサムウェアとランサム攻撃からの保護
- 必要な戦略は、拒否、検出、封じ込め、レスポンスとリカバリー
...
- SDLCの流れ：
  ...
  7. 修正とシステム交換
  8. 運用と保守
     - サービスレベル契約 (SLA; Service-Level Agreements) へのコンプライアンスを常に確認する必要がある
...
#### 統合生産プロセス開発 (IPPD)
- IPPD (Integrated Product and Process Development) は、システムプロジェクトにおいて、開発管理手法、製造アプローチ、試験・検証戦略の選択などの、プロジェクトがまだ明確に検討されていない段階ではできないことを認識しようとするもの
...
- OAuth 2 : 上記の進化版。署名が削除され、簡潔になった。TLSが必須
...
#### DMBS制御
- データベース内のデータの読み取り、書き込み、更新、照会、削除の制限をすること
...
- 継続的インテグレーション(CI; Continuous Integration)と継続的デリバリー(CD; Continuous Delivery)は、コーディングしてからデプロイするまでの様々なプロセスを自動化すること
- CI/CDが適切に実装されることで、DevSecOpsアプローチにもつながる
...
#### 正式なアプローチ
- ソフトウェアシステムのセキュリティシステム評価には以下の4つのタイプがある
  - **認証**と**認定** : ニーズや要件を分析し、必要な機能が正しく実装されていることを確認すること
  - **リスクマネジメントフレームワーク** : フレームワークを使うことでリスク管理と緩和の意思決定サイクルを網羅することができる
...
- 開発チームの最終テストが完了すると「認証」、運用チームの受け入れテストが完了すると「認定」される
...
- (参考) :
  ...
  - クリスタルボックステストは、ホワイトボックステストとも呼ばれる。有効性の検証
...
- NISTのリスクマネジメントフレームワークは、監査やコントロール評価を実施する際のスタンダードとして長く使われている

#### NISTサイバーセキュリティフレームワーク (NIST Cybersecurity Framework)
- CSF (CyberSecurity Framework) は、組織がサイバーセキュリティの回復力を評価するためのツールセットを提供する
- 検証されたリスク主導型の成熟度モデルを提供する
- 組織が、サイバーセキュリティのプラクティスを大規模な組織のリスク管理のプラクティスに統合することを可能にする
...
- SOC 1 と SOC 2 には、タイプが Type 1 と Type 2 の2種類ある
  - タイプIは、コントロールの「設計」や「適合性」を評価するもの
...
- トラストサービスクライテリアは、SOC 2, 3 で使われており、セキュリティ、可用性、処理の完全性、気密性、プライバシーなどを評価するための判断基準
...
#### 第三者による監査と評価
- クラウドサービスプロバイダー(CSP)やインターネットサービスプロバイダー(ISP)などと協力して、問題点を解決する場合もある
...
#### 評価のテーラリング
- 評価の範囲決定（スコーピング）とテーラリングには、現状を考慮したベースラインの更新やアップデートを含めること
...
- (参考)：
  ...
  - syslogは、重大度(Severity)の設定して送信するログを決める閾値の設定ができる
  - 重大度には、デバッグ、情報、注意、警告、エラー、致命的、警報、緊急 などがある
  - syslogのファシリティコード（ログメッセージの種類）は、どのような状況でログが発生したかを表す番号が指定される
  - ログは要塞ホスト (Bastion Host) と呼ばれる安全なログサーバに送信することで、ログを侵害から守ることができる
...
- ロールバックは、変更をモニタリングし、パフォーマンスが不十分と判断されたときに行う
...
- 組織はインシデント対処するための明確なポリシーを定めておく必要がある
...
- バックアップはCIAのうち可用性の側面で不可欠

#### 最低限の保護
- 3-2-1バックアップ戦略
  - 3つのデータコピー : 元データと2つのバックアップ
  - 2種類のストレージメディアタイプ : 磁気テープ、リムーバブルディスク、クラウド などにコピーする
  - 1つのオフサイトコピー : 十分離れた場所にコピーする
...
- 強迫(Duress)への対応とサイン（連絡手段）
- モバイルデバイス管理 (MDM) による紛失した・盗難されたデバイスのコンテンツ削除
...
- ギャップ分析は、評価・査定ツールとしてSCFを使用するためのアプローチを、半ば構造化された形で提供する
...
#### 事業継続計画および災害復旧計画の要素
- **事業継続計画**とは、本来サイトで何らかのインシデントが起きた時に以下を実施する計画：
  1. 本来サイトで事業を続けるために**緊急時対応計画**（コンティンジェンシープラン）を実施する
  2. 上で対応できないときは災害宣言をして**災害復旧計画**を実施し、バックアップサイトに移動する
  3. 最終的に本来サイトに戻るために**回復計画**を実施する
...
#### 審査・採用における特殊事情
- 会社の倒産によって、前職の情報が入手できず、審査・採用が十分に行えない状況も考えられる
...
#### コンプライアンスポリシー要件
- 組織は全ての担当者に対して許容範囲内の使用ポリシー (AUP; Acceptable Use Policies) を適用する
- AUPは、資産の適切かつ承認された使用方法をユーザの想定される観点から詳細に説明する

#### プライバシーポリシー要件
- 職員が PII (健康産業では PHI) にアクセスできる場合、取り扱いに関する組織のポリシーとプロシージャを文書化し、法的な影響を認識させることが不可欠

#### 秘密保持契約 (NDA; Non-Disclosure Agreements)
- 機密情報や営業秘密を保有する業界において、従業員が開示してはならないデータを指定して行う両当事者間の法的な合意
- NDAの種類：
  - 一方向のNDA
  - 相互NDA
  - 多角的NDA
...
- セキュリティコントロールアセスメント (SCA; Security Control Assessment) は、組織がリスク・脅威・統制プログラムに対して実施しなければならない定期的なレビューの一覧
  ...
  - セキュリティテストと評価 (ST&E) プロセスと組み合わされる
...
- A. 最低限の安全確保をして、ITを最大限に活用するため。対策費用が予想被害額よりも高くてはいけない
- Q. **どのように**セキュリティ対策をするのか？(**How**)
- A. 範囲を決めてガイドラインや国際的なスタンダードなどのベストプラクティスを実施してみる。そして、少しずつ手順などを組織に合うように調整していく
- Q. **どのくらい**セキュリティ対策が必要なのか？(**How much**)
- A. 攻撃を完全に防げる対策は存在しない。しかし、攻撃者を諦めさせる対策でも十分効果がある
